...
// //3."Write promise function to print numbers from 1 to 7, in which 1 should be printed after 1 sec ,
//  2 should be printed after 2 sec, 3 should be printed after 3 sec and so on. 
...
// //4.Create a promise function accepting a argument, if yes is passed to the function then it should 
// go to resolved state and print Promise Resolved, and if nothing is passed then it should go
//  to reject state and catch the error and print Promise Rejected .
...
function hello(x, y, sum) { // another  that is function2
    let result = x + y;
    sum(result);
}
hello(20, 30, names);// calling the function 1 in function2
...
const getEmpId = () => {
	setTimeout(() => {
		...
		setTimeout(() => {
			...
			console.log(empDetails.fName,empDetails.lName,empDetails.age);

			setTimeout(() => {
				(empDetails.gender = 'Male'),
					console.log(empDetails.fName,empDetails.lName,empDetails.age,empDetails.gender
					);
			}, 2000);
		}, 2000);
	}, 2000);
...
y.then((data)=>{console.log(data)}).catch((data)=>{console.log(data)})
console.log("After end of promise function")
